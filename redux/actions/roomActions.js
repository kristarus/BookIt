...
export const getRooms =
  ...
  async (dispatch) => {
    try {
      ...
      if (guests) {
        link.concat(`&guestCapacity=${guests}`);
      }
      if (category) {
...
export const newReview = (reviewData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(`${origin}/api/reviews`, reviewData, config);
    dispatch({
...
// new review
export const checkReviewAvailablity = (roomId) => async (dispatch) => {
  try {
    dispatch({ type: REVIEW_AVAILABILITY_REQUEST });

    const { data } = await axios.get(
      `${origin}/api/reviews/check_review_availablity?roomId=${roomId}`
    );
    dispatch({
...
export const getRoomReviwes = (id) => async (dispatch) => {
  try {
...
export const deleteRoom = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`${origin}/api/rooms/${id}`);
    dispatch({
